...
1. The Herdr plugin implements the persistent LazyVim pane, code comments sent
   to agents, mode theme, and targeted agent notifications.
...
- `setup/nvim/lua/plugins/review.lua`: ordinary LazyVim Diffview and Octo
  configuration, kept separate from the Herdr plugin. It maps `<leader>gv` to
  uncommitted changes, `<leader>gV` to branch changes, `<leader>gH` to current
  file history, and `<leader>gq` or `q` to close Diffview.
- `nvim/lua/plugins/herdr-lazyvim.lua`: the Herdr integration runtime only.
