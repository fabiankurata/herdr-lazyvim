...
The bundled Neovim integration adds a review hub, project navigation, Git and
pull-request views, and line comments that can be sent to any Herdr agent in the
workspace. It does not depend on `herdr-reviewr`.
...
- `jq` and `git`
- `gh` for pull-request information and GitHub Actions checks
...
Copy the LazyVim dependency spec so Diffview, Octo, and the review status are
available:
...
The integration is loaded only in editor processes opened by this plugin.
Useful mappings are:

| Mapping | Action |
| --- | --- |
| `<leader>ro` | Open the review hub |
| `<leader>r1` | Review the current change scope |
| `<leader>r2` / `<leader>rf` | Open the project explorer |
| `<leader>r3` / `<leader>rp` | Open pull-request tools |
| `<leader>rb` | Compare the branch with its base |
| `<leader>ru` | Review uncommitted changes |
| `<leader>rt` | Review the last captured agent turn |
