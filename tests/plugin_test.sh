...
# Worktree snapshots include staged, unstaged, and untracked files without
# changing the user's real Git index.
mkdir -p "$TMP/repo"
git -C "$TMP/repo" init -q
printf 'base\n' >"$TMP/repo/tracked.txt"
git -C "$TMP/repo" add tracked.txt
index_before=$(git -C "$TMP/repo" write-tree)
printf 'changed\n' >"$TMP/repo/tracked.txt"
printf 'new\n' >"$TMP/repo/untracked.txt"
snapshot_tree=$("$ROOT/scripts/herdr-review-snapshot" "$TMP/repo")
index_after=$(git -C "$TMP/repo" write-tree)
[ "$index_before" = "$index_after" ] || {
  printf 'FAIL: snapshot changed the real Git index\n' >&2
  exit 1
}
git -C "$TMP/repo" ls-tree --name-only "$snapshot_tree" | grep -Fx 'untracked.txt' >/dev/null
...
bash -n "$ROOT/herdr/config.sh" "$ROOT/herdr/launch.sh" "$ROOT/herdr/pane.sh" "$ROOT/herdr/notify.sh" "$ROOT/herdr/notification-focus.sh" "$ROOT/scripts/herdr-review-snapshot"
