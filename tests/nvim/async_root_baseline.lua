...
assert(after_a and #after_a.comments == 1, "baseline leaves acknowledged root A draft behind")
assert(after_b == nil, "baseline deletes unrelated root B state")
vim.fn.writefile({ vim.json.encode({ calls = calls, result = "baseline defect reproduced" }) }, fixture .. "/async-root-baseline.json")
print("async root baseline: reproduced")
vim.cmd("qa!")
